...
- **禁止在修复任何测试失败后，不重新运行全量测试套件就标记"已完成"。** 修了一个失败，必须重新跑全量，确认没有引入新问题。只重新跑被修的那几个测试不算验证。
...
在 grep/find 盲搜代码之前，先查此表。问下面这些问题时，对应的文档在一秒内能给出答案：

| 你想知道 | 先看这份文档 | 为什么 |
|----------|-------------|--------|
| 前端/后端代码在哪、目录结构 | `docs/当前实现总览.md` | 第 6 节有完整的 React 前端目录、构建产物路径、路由→源文件对照表 |
| 某个 API 的请求/响应格式 | `docs/当前实现总览.md` 或 `docs/modules/` 对应模块 | 模块文档记录接口契约 |
| 某个表的结构、字段含义 | `docs/modules/` 对应模块文档 | 每个模块文档有数据模型章节 |
| 这个项目怎么跑起来 | `docs/开发文档索引.md` | 指向部署、配置、环境相关文档 |
| 架构/技术选型/模块关系 | `docs/设计总览.md` | 技术栈、模块地图、核心业务流 |
| 最近改了什么、为什么改 | `CHANGELOG.md` | 每次改动都记，按日期排列 |
| 某个 backlog item 具体要做什么 | `docs/backlog/<NN>-<模块>.md` | 范围、验收条件、依赖 |
| 某个功能的设计决策记录 | `plans/<date>-<topic>.md` | 实现方案、风险、验证记录 |

**规则**：当需要定位文件路径、了解模块职责、确认 API 契约时，先查上表对应的文档。文档里已经有答案的情况下，不允许用 grep/find 盲搜。读文档 → 精准定位 → 再读代码。
...
8. Data flow trace has been run and all nodes pass, deslop pass (ai-slop-cleaner) has been run on changed files, and post-deslop full regression passes; or the user explicitly waived these steps.
...
- 检查某模块"有没有遗漏的问题"或"还有什么欠缺"
- 只出报告，不直接修改代码

### 核心原则

**先出待处理清单，再逐条与用户确认，不跳过用户直接进入实现。**

很多审计发现的问题并非真的需要修——有的是用户刻意为之，有的概率极低不值得修，有的是当时的设计决策。AI 的报告只是起点，最终修哪些由用户决定。

### Steps

**Step 1 — 确认审计范围**：最近 N 天、指定模块、指定 plans、是否包含当前工作区未提交改动。

**Step 2 — 加载上下文**（最小包）：`docs/项目控制台.md`、相关 `plans/*.md`、相关 `docs/backlog/*.md`、相关 `docs/modules/*.md`、近期改动文件和测试。

**Step 3 — 执行审计**：审计代码、测试、文档、状态一致性。可启动多个并行子 agent 分维度探索。

**Step 4 — 产出待处理清单**：写入 `plans/<date>-<topic>-审计待处理清单.md`。
...
| 位置 | 文件路径和行号 |
| 现象 | 代码实际行为和预期行为的差异 |
| 场景举例 | **必填**。用具体数据从前到后描述什么情况下会触发、用户会看到什么结果、有什么实际影响。不用术语，用场景说明 |
| 修复方向 | 建议怎么做（一句话） |
| 修复效果 | 修复后预期达到的效果 |

场景举例要求：每一条的"场景举例"必须让一个不了解这块代码的人一眼看懂"**如果我不修这个问题，某天用户做了什么操作，就会发生什么**"。禁止只写技术描述不写场景。

示例（好的场景举例）：
> 用户有一个批量任务，跑了 5000 个目标。用户点了暂停 → 生产者停了，但 10 个 consumer 线程还在按正常速度处理队列里积压的 1000 条目标（每条要 sleep 2 秒）。等了 3 分钟还没停 → 用户以为功能坏了。

示例（不好的场景举例）：
> consumer_exp 不检查 pause_requested。 ← 这只是技术描述，用户看不懂

**Step 5 — 逐条确认**：按致命→高→中→低顺序，逐条向用户展示，等待用户决定（修 / 不修 / 怎么修）。每条确认后立即更新清单中该条的状态。用户说"可以""没问题""就按这个"即视为确认。

严禁以下行为：
- **禁止跳过确认直接实现**：清单只是草案，最终执行必须经过用户
- **禁止连珠炮式提问**：每条单独展示，用户确认后再展示下一条
- **禁止把技术描述当场景举例**：每条必须有具体用户操作和后果
- **禁止在用户未看完清单前就开始修**：即使某条看起来"显而易见"，也必须等用户确认

**Step 6 — 整理最终执行清单**：全部确认完毕后，在清单末尾追加排序后的执行列表（按优先级和依赖排序），明确标注哪些修、哪些跳过、哪些已定方案。

**Step 7**：🛑 STOP。除非用户明确说"开始实现"或"继续"，否则不进入 Mode D。

如果用户说"开始实现"，按最终执行清单逐项进入 Mode D 或 Mode D-lite。
...
然后等待用户回复。用户确认后，简短更新状态并展示下一条。

### Success criteria

- 每条问题有具体的场景举例（用户看完就知道什么时候会触发）
- 用户逐条确认了修/不修/怎么修
- 最终清单直接可用作实现队列
- 报告和确认为主的对话结束后，不自动进入实现
...
1. Read `docs/项目控制台.md`. **在接触源码之前**，先查 `docs/当前实现总览.md` 获取模块目录结构和文件路径（见上方快速定位表）。已知道文件位置后再读源码/测试。
2. **先过编码前必答清单**（见上方）。
...
5. TDD（非 CSS/布局改动时）：先写测试→确认失败→最小实现→测试全绿。
6. Implement the minimum fix only.
7. Run targeted verification and record any pre-existing failures separately from new failures.
...
4. Read relevant code and tests. **在此之前**：若不确定相关文件位置，先查 `docs/当前实现总览.md` 和 `docs/开发文档索引.md` 获取路径和模块职责。不盲搜。
...
10. **TDD（红→绿→重构）**：
    a. **红**：先写测试，覆盖每个验收条件。运行测试确认它们失败，且失败原因正确（不是因为语法错误或 mock 不当）。如果现有测试文件位置不明确，在 `tests.py` 中追加。
    b. **绿**：实现最少代码让所有测试通过。只改必须改的，不过度工程化。
    c. **重构**：测试全绿后，审视代码——消除重复、改善命名、简化逻辑。每次重构后重新跑测试保持全绿。
    豁免：纯前端 CSS/布局改动（不涉及行为/数据流）可跳过 TDD，但仍需 Step 11 的四层验证。
11. **Verification**:
    ...
    e. **Data flow trace（数据流走查）**：验证功能在真实执行路径上是否完整贯通——不是"代码写得对不对"，而是"有没有代码来处理这件事"。reviewer 审质量，data flow trace 审完整性。

       1. **写场景**：一句话描述用户操作和期望结果。必须具体、可执行。例如："用户在自动扫描任务创建页，选择输入源为'fscanx输出文件'，上传文件，选覆盖策略，点保存。预期任务创建成功。"

       2. **追踪链路**：从场景出发，按执行顺序列出数据经过的每一站。覆盖所有层——前端组件 → HTTP 请求 → 路由 → 视图函数 → 表单校验 → 业务逻辑分发 → 数据持久化 → 响应 → 前端渲染。

       3. **逐站检查**：对每一站，打开源文件确认这四项：
          - **数据到达**：这一站收到新功能相关的字段/参数了吗？
          - **逻辑存在**：有代码处理这个新值吗？（不是被漏掉，不是只写了 TODO）
          - **分支正确**：走的是正确的分支，不是被 default/else/fallback 吞掉？
          - **无缺字段错**：不会因为缺字段、缺参数而崩溃或静默跳过？

       4. **发现问题立即修复**，修完重走该站及后续站点。

       5. **输出**：链路清单 + 每站结果（✓ 通过 / ✗ 已修复 / — 不适用）。全部 ✓ 后进入下一步。

    f. **Full regression re-run（硬闸门）**：Run the ENTIRE test suite — not just the tests you touched, not just the modules you changed. The full `python manage.py test` (or project equivalent). Read the output carefully.
       - If failures appear: classify each as (A) caused by this change → fix and re-run full suite from scratch, or (B) pre-existing → verify by stashing and running the same test on the original code. Record the evidence.
       - This step is NOT satisfied by running only the tests you "think are relevant." Cross-module changes can break tests in other files.
       - A test count drop from the pre-change baseline is a regression even if no individual test reports FAIL.
       - After any fix, re-run the full suite. Never assume the fix didn't break something else.
12. Update:
...
- Every acceptance criterion has an automated test that passes.
- New tests were written first, confirmed failing for the right reason, then made green by implementation (TDD).
...
- Implement selected items with verification loop (Mode D).
...
- Treating reviewer approval as the final step — data flow trace, deslop, and regression must follow in the same turn.
...
- 跳过 TDD 步骤直接写实现——测试不是事后补的，是实现前的红绿灯。写完测试确认失败原因正确，再开始实现。
...
- 用 grep/find 盲搜项目结构而不是先查 `docs/当前实现总览.md`——文档里已经有路径和职责说明，盲搜又慢又不准。
- **修复失败后只重跑修过的那几个测试，不跑全量就报告完成**——每次修复都可能引入新问题。修完必须重新跑全量套件。之前确认是预存的失败也要在最终报告中逐一列出（测试名、失败摘要、预存证据），不能一笔带过"都是预存的"。
...
| 全量回归测试出现失败 | 逐条分类：A（本次改动引入，立即修复）→ 修复后重新跑全量；B（预存失败，stash 验证）→ 记录测试名、失败摘要、预存证据 | 仍有 A 类无法修复 → 🛑 STOP，汇报用户。B 类混杂无法区分 → stash 后跑全量对比基线 |
...
- 已完成：
- 修改文件：
- 验证（自检 + reviewer + deslop + flow trace + 回归）：
- 未完成/风险：
- 建议下一步：
```
